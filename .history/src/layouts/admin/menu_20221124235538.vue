<template>
      ...
      <div class="left-container">
        <dl v-for="(menu, index) of menus.menus" :key="index">
          <dt @click="handle(menu)">
            <div>
              <i :class="menu.icon" class="text-[15px]"></i>
              <span class="ml-2 text-[14px]">{{ menu.title }}</span>
            </div>
            <i class="fas fa-angle-down duration-500" :class="{'rotate-180':menu.isClick}" ></i>
          </dt>
          <dd
            v-show="menu.isClick"
...
console.log(typeof menus.menus)

// const handle = (pmenu:IMenu , cmenu?:RouteRecordRaw ) => {
//   // resetMenu();
